...
from dataset import WikiDataset,StreamingWikiDataset,DistributedStreamingWikiDataset
...
def main():
   ...
   parser.add_argument('--warmup-steps', help='number of steps to warm up learning rate', type=int, default=5000)
   parser.add_argument('--total-steps', help='number of steps to decay learning rate', type=int, default=1e5)
   ...
   dataset = DistributedStreamingWikiDataset(train_file, rank=rank, world_size=world_size)
   logger.info(f'vocab size: {args.vocab_size}')
   model = NGramLanguageModeler(args.vocab_size, embedding_dim, dataset.context_size).to(device)
